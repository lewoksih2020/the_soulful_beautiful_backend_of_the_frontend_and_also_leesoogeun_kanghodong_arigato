...
class CommentSerializer(serializers.ModelSerializer):
    ...
    loanrequest = LoanrequestSerializer(
        read_only=True
    )
    ...
    class Meta:
        model = Comment
        fields = (
            'loanrequest', 'authorsender', 'parent', 'body', 'upvotes',
    ...
    def create(self, validated_data):
        ...
        if int(parent_fn[1]) == 1:
            ...
            try:
                validated_data['loanrequest'] = Loanrequest.objects.get(
                    pk=validated_data['parent'].loanrequest_id
                )
            ...
            try:
                validated_data['loanrequest'] = Loanrequest.objects.get(pk=parent_pk)
            except Loanrequest.DoesNotExist:
                error_message = _((
                    "The post you are replying to is no "
                    "longer available"
    ...
    def reddit_delete(self):
        ...
        authorsender = getattr(instance, 'authorsender')
        setattr(instance, 'authorsender', None)
...
class CommentauthorsenderSerializer(serializers.ModelSerializer):
    class Meta:
        model = User
...
class CommentTreeSerializer(serializers.ModelSerializer):
    ...
    authorsender = CommentauthorsenderSerializer()
    created = serializers.SerializerMethodField()
    ...
    class Meta:
        model = Comment
        fields= (
            'loanrequest', 'body', 'upvotes', 'parent',
            'created', 'vote_state', 'deleted', 'pk', 'authorsender', 'children',
        )
