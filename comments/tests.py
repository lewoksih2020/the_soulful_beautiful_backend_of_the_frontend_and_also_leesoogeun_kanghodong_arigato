...
class CommentTest(APITestCase):
    ...
    def create_comment(self, **kwargs):
        return Comment.objects.create(
            authorsender=kwargs.get("authorsender"),
            loanrequest=kwargs.get("loanrequest"),
    ...
    def test_root_comment_creation(self):
        ...
        self.assertEqual(comment.authorsender, self.user)
        self.assertEqual(comment.loanrequest, self.loanrequest)
    ...
    def test_child_comment_creation(self):
        ...
        self.assertEqual(comment.authorsender, self.user)
        self.assertIsNone(comment.loanrequest)
        self.assertEqual(len(comment.children.all()), 0)
...
class CommentViewTests(APITestCase):
    ...
    def test_root_comment_creation(self):
        ...
        parent_fn = "t2_{}".format(self.loanrequest.pk)
        data = {
            "parent_fn": parent_fn,
        ...
        self.assertEqual(Comment.objects.count(), 1)
        self.assertEqual(response.data["loanrequest"]["pk"], self.loanrequest.pk)
    ...
    def test_child_comment_creation(self):
        ...
        response = self.client.post(self.comment_list_url, data)
        self.assertContains(response, self.loanrequest.title, status_code=201)
        self.assertEqual(Comment.objects.count(), 2)
        self.assertEqual(response.data["body"], self.comment_body)
    ...
    def test_root_comment_deletion(self):
        ...
        self.assertIsNone(response.data["authorsender"])
    ...
    def test_child_comment_deletion(self):
        ...
        self.assertEqual(
            gc_response.data["loanrequest"],
            response.data["loanrequest"]
        )
        self.assertEqual(
            gc_response.data["loanrequest"],
            root_response.data["loanrequest"]
        )
        self.assertEqual(
            gc_response.data["loanrequest"]["pk"],
            self.loanrequest.pk
        )
...
class SeedCommentsSubredditCommandTests(TestCase):
    def setUp(self):
        # need to get users, subreddits and loanrequests first
        out = StringIO()
        call_command('seed_users', number=5, stdout=out)
        call_command('seed_subreddits', number=5, stdout=out)
        call_command('seed_loanrequests', number=10, stdout=out)
    ...
    def test_seed_child_comments(self):
        ...
        for comment in Comment.objects.all():
            self.assertIsNotNone(comment.loanrequest)
